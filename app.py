import streamlit as st
import pandas as pd
import joblib
import matplotlib.pyplot as plt
import seaborn as sns  

# Membaca data dari CSV
df = pd.read_csv('data_hasil.csv')  # Gantilah dengan path ke file kamu

# ========== Page Setup ==========
st.set_page_config(
    layout="wide",
    page_title="Prediksi Umrah",
    page_icon="🕋"
)

# ========== Styling ==========
st.markdown("""
    <style>
    * {
        font-family: 'Times New Roman', serif !important;
    }
    html, body, [class*="css"] {
        font-size: 18px;
    }
    label {
        font-size: 1.25rem !important;
    }
    .stButton>button {
        background-color: #FFD700;
        color: black;
        font-weight: 600;
        border-radius: 8px;
        font-size: 1.1rem;
        padding: 0.6em 2.2em;
        border: none;
        display: block;
        margin: 0 auto;
    }
    .stButton>button:hover {
        background-color: #FFEA94;
    }
    .main-title {
        text-align: center;
        color: #000;
        font-size: 2.3rem;
        font-weight: bold;
        margin-bottom: 0.2em;
    }
    .sub-title {
        text-align: justify;
        font-size: 1.1rem;
        color: #333;
        margin-bottom: 1rem;
        padding: 0 4rem;
    }
    .quote {
        text-align: center;
        font-style: italic;
        color: #555;
        font-size: 1rem;
        margin-bottom: 2rem;
    }
    hr {
        border: none;
        height: 2px;
        background-color: #C8A200;
        margin-top: 10px;
        margin-bottom: 30px;
    }
    </style>
""", unsafe_allow_html=True)

# ========== Header ==========
header1, header2 = st.columns([1, 16])
with header1:
    st.image("logo_babul.png", width=60)
with header2:
    st.markdown("""
    <div style='display: flex; align-items: center; height: 100%;'>
        <h1 style='color:#A62639; font-size: 28px; margin: 0;'>BABUL KA'BAH</h1>
    </div>
    """, unsafe_allow_html=True)

st.markdown("<hr>", unsafe_allow_html=True)

# ========== Title & Sub ==========
st.markdown("<h2 class='main-title'>REKOMENDASI PAKET UMRAH</h2>", unsafe_allow_html=True)
st.markdown("<p class='sub-title'>Sistem ini dirancang untuk membantu memilih paket terbaik berdasarkan preferensi dan profil jamaah. Isi data calon jamaah untuk mendapatkan rekomendasi paket umrah yang sesuai kebutuhan.</p>", unsafe_allow_html=True)
st.markdown("""
<p class='quote'>
    "Dan sempurnakanlah ibadah haji dan umrah karena Allah."<br>
    – <em>QS. Al-Baqarah: 196</em>
</p>
""", unsafe_allow_html=True)

# ========== Load Model & Label Encoders ==========
model = joblib.load('model_umrah_rf.joblib')
label_encoder = joblib.load('label_encoder_umrah.joblib')
fit_columns = joblib.load('fit_columns.joblib')

# ========== Sidebar for Method Selection ==========
sidebar_option = st.sidebar.radio(
    "Pilih Metode",
    ("Input Manual", "Visualisasi Histori")
)

# Input Manual
if sidebar_option == "Input Manual":
    with st.form("form_umrah"):
        form_col1, form_col2 = st.columns(2)

        with form_col1:
            jenis_kelamin = st.selectbox("Jenis Kelamin", ["Laki-laki", "Perempuan"])
            usia = st.number_input("Usia", min_value=18, max_value=100, value=30)
            wilayah_options = [
                "Jawa", "Sumatera", "Sulawesi", "Kalimantan",
                "Maluku", "Bali & Nusa Tenggara", "Papua"
            ]
            wilayah_geografis = st.selectbox("Wilayah Geografis", wilayah_options)

        with form_col2:
            tahun = st.selectbox("Tahun Keberangkatan", [2022, 2023])
            bulan = st.slider("Bulan Keberangkatan", 0, 12, 5)
            tanggal = st.slider("Tanggal Keberangkatan", 1, 31, 15)

        st.markdown("<br>", unsafe_allow_html=True)
        submit = st.form_submit_button("🔎 Lihat Rekomendasi")

    # Proses Prediksi
    if submit:
        # Encoding input hanya jika kolomnya masih kategorikal
        jk_encoded = 0 if jenis_kelamin == "Laki-laki" else 1
        wilayah_mapping = {name: i for i, name in enumerate(wilayah_options)}
        wilayah_encoded = wilayah_mapping[wilayah_geografis]

        # Membuat DataFrame dengan input yang diberikan oleh user
        input_data = pd.DataFrame({
            'jenis_kelamin': [jk_encoded],
            'usia': [usia],
            'wilayah_geografis': [wilayah_encoded],
            'tahun': [tahun],
            'bulan': [bulan],
            'tanggal': [tanggal]
        })

        # Menyelaraskan urutan kolom dengan yang ada di fit_columns
        input_data = input_data[fit_columns]
        
        # Melakukan prediksi dengan model yang sudah dilatih
        prediksi = model.predict(input_data)
        hasil = label_encoder.inverse_transform(prediksi)

        st.success(f"Paket Umrah yang cocok untuk pelanggan ini adalah: **{hasil[0]}**")

        # Menampilkan probabilitas untuk setiap paket
        proba = model.predict_proba(input_data)[0]
        prob_df = pd.DataFrame({
            'Paket': label_encoder.classes_,
            'Probabilitas': proba
        }).sort_values(by='Probabilitas', ascending=False)

        # ===== Tabel & Chart Sejajar =====
        col_tabel, col_chart = st.columns([1, 1.2], gap="large")

        with col_tabel:
            st.markdown("<h4 style='margin-bottom: 0.5em;'>📊 Rincian Probabilitas:</h4>", unsafe_allow_html=True)
            st.dataframe(
                prob_df.set_index('Paket').style.format({"Probabilitas": "{:.2%}"}), 
                use_container_width=True,
                hide_index=False
            )

        with col_chart:
            st.markdown("<h4 style='margin-bottom: 0.5em;'>📈 Visualisasi Probabilitas:</h4>", unsafe_allow_html=True)
            fig, ax = plt.subplots(figsize=(8.5, 4.5))
            bars = ax.bar(prob_df['Paket'], prob_df['Probabilitas'], color='#4F7942')

            for bar in bars:
                height = bar.get_height()
                ax.annotate(f'{height:.1%}',
                            xy=(bar.get_x() + bar.get_width() / 2, height),
                            xytext=(0, 8),
                            textcoords='offset points',
                            ha='center', va='bottom',
                            fontsize=10,
                            fontname='Times New Roman')

            ax.set_title("Visualisasi Probabilitas Paket Umrah", fontsize=14, fontweight='bold', pad=15, fontname='Times New Roman')
            ax.set_ylabel("Probabilitas", fontsize=12, fontname='Times New Roman')
            ax.set_xlabel("Nama Paket", fontsize=12, fontname='Times New Roman')
            ax.tick_params(axis='x', labelrotation=20)
            ax.grid(axis='y', linestyle='--', alpha=0.6)
            ax.set_axisbelow(True)

            for label in ax.get_xticklabels() + ax.get_yticklabels():
                label.set_fontname('Times New Roman')

            plt.tight_layout()
            st.pyplot(fig)


# ========== If "Visualisasi Histori" is selected ==========
elif sidebar_option == "Visualisasi Histori":
    st.write("Visualisasi Grafik Berdasarkan Data Histori")

    # Membuat 2 kolom dan 3 baris untuk menata grafik
    col1, col2 = st.columns(2)

    # Baris pertama grafik
    with col1:
        # 1. Distribusi Paket Umrah
        st.subheader("Distribusi Paket Umrah")
        plt.figure(figsize=(6, 4))  # Sesuaikan ukuran grafik
        sns.countplot(data=df, x='paket_umrah', palette='tab20b')
        plt.title('Distribusi Paket Umrah (Target)', fontsize=10)
        plt.xlabel('Jenis Paket Umrah', fontsize=9)
        plt.ylabel('Jumlah', fontsize=9)
        plt.xticks(rotation=45)
        plt.tight_layout()
        st.pyplot(plt)

    with col2:
        # 2. Distribusi Usia
        st.subheader("Distribusi Usia")
        plt.figure(figsize=(6, 4))  # Sesuaikan ukuran grafik
        sns.histplot(df['usia'], bins=10, kde=True, color='skyblue')
        plt.title('Distribusi Usia', fontsize=10)
        plt.xlabel('Usia', fontsize=9)
        plt.ylabel('Jumlah', fontsize=9)
        plt.tight_layout()
        st.pyplot(plt)

    # Baris kedua grafik
    with col1:
        # 3. Bulan-Tahun Pemesanan
        st.subheader("Bulan-Tahun Pemesanan")
        plt.figure(figsize=(6, 4))  # Sesuaikan ukuran grafik
        sns.countplot(data=df, x='bulan_tahun', color='skyblue')
        plt.title('Bulan-Tahun Pemesanan', fontsize=10)
        plt.xlabel('Bulan-Tahun Pemesanan', fontsize=9)
        plt.ylabel('Jumlah', fontsize=9)
        plt.xticks(rotation=45)
        plt.tight_layout()
        st.pyplot(plt)

    with col2:
        # 4. Pemesanan Paket Umrah per Bulan
        st.subheader("Pemesanan Paket Umrah per Bulan")

        # Convert 'tanggal_pemesanan' to datetime and extract the month
        df['tanggal_pemesanan'] = pd.to_datetime(df['tanggal_pemesanan'])
        df['bulan'] = df['tanggal_pemesanan'].dt.to_period('M')

        # Pivot the data to get counts of 'tanggal_pemesanan' by 'paket_umrah' per month
        pivot_data = df.groupby(['bulan', 'paket_umrah'])['tanggal_pemesanan'].count().unstack().fillna(0)

        # Reindex to include all months from Dec 2022 to Dec 2023
        bulan_order = pd.period_range(start="2022-12", end="2023-12", freq="M")
        pivot_data = pivot_data.reindex(bulan_order.astype(str), fill_value=0)

        # Define custom colors for the stacked bars
        formal_colors = ['#5d7290', '#d9a066', '#a3b18a', '#c97c7c', '#f1e0c5', '#9a8f97']

        # Create the figure and axis for the plot with adjusted size
        fig, ax = plt.subplots(figsize=(6, 4))  # Adjusted size here

        # Plot the data as a stacked bar chart
        pivot_data.plot(kind='bar', stacked=True, color=formal_colors, ax=ax)

        # Add labels inside the bars
        for container in ax.containers:
            for rect in container:
                height = rect.get_height()
                if height > 0:
                    # Choose the text color based on the bar color brightness
                    text_color = 'white' if rect.get_facecolor()[:3] < (0.3, 0.3, 0.3) else 'black'
                    ax.text(
                        rect.get_x() + rect.get_width()/2,
                        rect.get_y() + height - (height * 0.4),
                        int(height),
                        ha='center', va='center', fontsize=7, color=text_color, fontweight='bold'
                    )

        # Add title and labels
        plt.title('Pemesanan Paket Umrah per Bulan', fontsize=12)
        plt.xlabel('Bulan Pemesanan', fontsize=10)
        plt.ylabel('Jumlah Pemesanan', fontsize=10)

        # Rotate x-axis labels for readability
        plt.xticks(rotation=45)

        # Display the plot in Streamlit
        st.pyplot(fig)

    # Baris ketiga grafik
    with col1:
        # 5. Pemesanan Paket Berdasarkan Kelompok Usia
        st.subheader("Pemilihan Paket Berdasarkan Kelompok Usia")
        bins = [0, 18, 27, 37, 47, 57, 67, 77]
        labels = ["<18", "18-27", "28-37", "38-47", "48-57", "58-67", "68-77"]
        df['kelompok_usia'] = pd.cut(df['usia'], bins=bins, labels=labels, right=False)

        pivot_data = df.groupby(['kelompok_usia', 'paket_umrah']).size().unstack().fillna(0)

        formal_colors = ['#4e79a7','#f28e2b', '#76b7b2', '#e15759', '#a0cbe8',  '#ffbe7d']
        fig, ax = plt.subplots(figsize=(8, 6))
        pivot_data.plot(kind='bar', stacked=True, color=formal_colors, ax=ax, alpha=0.85)

        for container in ax.containers:
            for rect in container:
                height = rect.get_height()
                if height > 0:
                    text_color = 'white' if rect.get_facecolor()[:3] < (0.3, 0.3, 0.3) else 'black'
                    ax.text(
                        rect.get_x() + rect.get_width()/2,
                        rect.get_y() + height - (height * 0.4),
                        int(height),
                        ha='center', va='center', fontsize=7, color=text_color, fontweight='bold'
                    )

        plt.title("Pemilihan Paket Berdasarkan Kelompok Usia", fontsize=12)
        plt.xlabel("Kelompok Usia", fontsize=10)
        plt.ylabel("Jumlah Pemesanan", fontsize=10)
        plt.xticks(rotation=0)
        plt.legend(title="Jenis Paket", loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3)
        plt.grid(axis='y', linestyle='--', alpha=0.6)
        st.pyplot(fig)

    with col2:
        # 6. Pemesanan Paket Berdasarkan Jenis Kelamin
        st.subheader("Pemesanan Paket Berdasarkan Jenis Kelamin")
        pivot_gender = df.groupby(['paket_umrah', 'jenis_kelamin']).size().unstack().fillna(0)

        gender_colors = ['#4e79a7', '#f3a7db']

        fig, ax = plt.subplots(figsize=(8, 6))
        pivot_gender.plot(kind='bar', color=gender_colors, ax=ax)

        plt.title("Pemesanan Paket Umrah Berdasarkan Jenis Kelamin", fontsize=12)
        plt.xlabel("Jenis Paket Umrah", fontsize=10)
        plt.ylabel("Jumlah Pemesanan", fontsize=10)
        plt.xticks(rotation=15, fontsize=9, ha='center')

        plt.legend(title="Jenis Kelamin", labels=["Pria", "Wanita"], bbox_to_anchor=(1, 1), loc='upper left')

        for container in ax.containers:
            ax.bar_label(container, fmt='%d', label_type='edge', fontsize=9, color='black', fontweight='bold')

        plt.grid(axis='y', linestyle='--', alpha=0.6)

        plt.tight_layout()
        st.pyplot(fig)
